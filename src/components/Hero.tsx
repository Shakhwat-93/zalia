...
const Hero3DModel = dynamic(() => import('@/components/3d/Hero3DModel'), {
  ...
  loading: () => (
    <div className="w-full h-full flex flex-col items-center justify-center space-y-3">
      ...
      <span className="text-[11px] font-mono text-charcoal-400 uppercase tracking-widest">
        Loading 3D Scene...
      </span>
    </div>
  ),
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Right Column: Unified Architectural Showcase (Image Render or Interactive 3D) */}
        <div className="lg:col-span-6 relative w-full h-[460px] sm:h-[560px] lg:h-[620px] flex items-center justify-center">
          ...
          >
            ...
            <div className="absolute top-4 left-4 right-4 z-20 flex items-center justify-between pointer-events-none">
              <div className="inline-flex items-center space-x-2 px-3 py-1.5 rounded-full bg-white/90 backdrop-blur-md border border-canvas-border text-[10px] font-mono uppercase tracking-widest text-charcoal-700 shadow-sm pointer-events-auto">
                <Sparkles className="w-3 h-3 text-gold-accent" />
                <span>ARCHITECTURAL SPECIFICATION</span>
              </div>

              {/* Seamless Render vs 3D Switcher */}
              <div className="inline-flex items-center p-1 rounded-full bg-white/95 backdrop-blur-md border border-canvas-border shadow-sm pointer-events-auto">
                <button
                  onClick={() => setActiveTab('render')}
                  className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-full text-[11px] font-mono uppercase tracking-wider transition-all duration-300 ${
                    activeTab === 'render'
            ...
            {/* Display Area: Mode 1 Render or Mode 2 Interactive 3D */}
            ...
            {/* Bottom Glass Metadata Strip */}
            <div className="absolute bottom-4 left-4 right-4 z-20 p-3.5 rounded-2xl glass-card border border-white/60 flex items-center justify-between shadow-soft-sm backdrop-blur-md">
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 rounded-lg bg-emerald-brand text-white flex items-center justify-center shadow-xs">
                  <Compass className="w-4 h-4" />
                </div>
                <div>
                  <h4 className="text-[11px] font-semibold text-charcoal-900 uppercase tracking-wider">
              ...
              <span className="text-[10px] font-mono font-medium px-2.5 py-1 rounded bg-canvas-warm text-charcoal-700 border border-canvas-border">
